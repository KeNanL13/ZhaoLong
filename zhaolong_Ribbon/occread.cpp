...
//    LabelPrsMap mapOfOriginals;
     ...
     for ( TDF_LabelSequence::Iterator lit(roots); lit.More(); lit.Next() )
     {
       ...
       mapOfItemToLabel.insert(toQString(name1->Get()),L);
       //遍历子节点，找到所有形状
...
Handle(TDocStd_Document)occRead::getDoc()
{
    return m_doc;

}

QStandardItem *occRead::getTreeNode()
{
    return m_treeNode;
}
...
void occRead::displayItem(const TDF_Label &label, const TopLoc_Location &parentTrsf, const XCAFPrs_Style &parentStyle, const TCollection_AsciiString &parentId, LabelPrsMap &mapOfOriginals, QStandardItem *parentItem)
{
    ...
    TDF_Tool::Entry(refLabel, itemId);
//    qDebug()<<this->toQString(itemId);
    //判断标签是否是Assembly
...
//          mapOfLocation.insert(brepConnected,parentTrsf);

          NCollection_List<Handle(AIS_InteractiveObject)>*aisList = mapOfPresentions.ChangeSeek(label);
          if(aisList==nullptr)
          {
              NCollection_List<Handle(AIS_InteractiveObject)> itemList;
              itemList.Append(brepConnected);
              mapOfPresentions.Bind(label,itemList);
          }
          else {
              *aisList->Append(brepConnected);
          }
    ...
    for ( TDF_ChildIterator childIt(refLabel); childIt.More(); childIt.Next() )
    {
      ...
      qDebug()<<toQString(name1->Get());
      QStandardItem * item=new QStandardItem(toQString(name1->Get()));
      parentItem->appendRow(item);
      mapOfItemToLabel.insert(item->text(),childLabel);
