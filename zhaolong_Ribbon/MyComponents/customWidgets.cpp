...
singleSelectGroup::singleSelectGroup(QString name, QWidget *parent) :
    QWidget(parent){
    QFont titleFont = QFont("Corbel", 16,QFont::Bold);
    QFontMetrics fm(titleFont);
    qreal height = fm.lineSpacing();
...
textInputItem::textInputItem(const QString &name, QWidget *parent) :
    QWidget(parent)
{
    QFont nameFont = QFont("Font Awesome 6 Free Solid", 12);
    ...
    QFont textFont = QFont("Font Awesome 6 Free Solid", 12);
...
void textInputItem::enterEditEffect(){
    ...
    qDebug()<<QRectF(this->width() * 0.3, this->height() / 2 - editor->height() / 2 - 2, this->width() * 0.7 - margin, editor->height());
